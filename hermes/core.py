...
class TooManFeatures(ValueError):
    ...
    def search_algo(self, algo: Callable, *args, allow_this_many=1) -> list:
        ...
        if isinstance(result_algo, list):
            if len(result_algo) > allow_this_many:
                raise TooManyFeatures(f'{algo.__name__} = {result_algo}',
                                      self.text)
            return result_algo
...
def search_close(text: str):
    # manual check close
    blacklist = [
        'if', 'candle', 'closer', 'will', 'not', 'might', 'minute', 'daily',
        'support', 'been'
    ]
...
def interpret(text: str) -> dict:
    ...
    if data['symbol'] and data['sl'] and data['side']:
        ...
        try:
            data['flag'] = [x for x in data if (data[x] and x in allowed)
                            ][0].upper()
        except IndexError:
            data['flag'] = None
